...
public class InterMunicipaisUrbanas {
	...
	CaronaMunicipalBusiness municipalBusiness;
	...
	public void inicializar() {
		...
		municipalBusiness = new CaronaMunicipalBusiness();
	...
	public void cadastrarCaronas() {
		...
		try {
			assertEquals("3", municipalBusiness.cadastrarCaronaMunicipal("mark",
		...
		try {
			assertEquals("4", municipalBusiness.cadastrarCaronaMunicipal("mark",
		...
		try {
			assertEquals("5", municipalBusiness.cadastrarCaronaMunicipal("mark",
		...
		try {
			assertEquals("{3m}", municipalBusiness.localizarCaronaMunicipal("bill", "Campina Grande", "Açude Velho", "Shopping Boulevard"));
		} catch (CaronaException e) {
			fail();
		}
		
		try {
			assertEquals("{4m,5m}", municipalBusiness.localizarCaronaMunicipal("mark", "João Pessoa"));
		} catch (CaronaException e) {
			fail();
		}
		...
		try {
			municipalBusiness.localizarCaronaMunicipal("bill", null, "Açude Velho", "Shopping Boulevard");
		} catch (CaronaException e) {
			assertEquals("Cidade inexistente", e.getMessage());
		}
		 
		try {
			municipalBusiness.localizarCaronaMunicipal("bill", "", "Açude Velho", "Shopping Boulevard");
		} catch (CaronaException e) {
			assertEquals("Cidade inexistente", e.getMessage());
		}
